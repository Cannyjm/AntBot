...
class VideoCanvas extends JPanel implements MouseListener
{
	...
	public synchronized void paintComponent(Graphics g)
	{
		...
		image = Mat2BufferedImage( displayFrame );
	...
	{
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH_mm_ss_SSSS");
		Date date = new Date();
		
		Container c = this;
		BufferedImage im = new BufferedImage(c.getWidth(), c.getHeight(), BufferedImage.TYPE_INT_ARGB);
		c.paint(im.getGraphics());
		File saveFile = new File("Pictures\\"  + dateFormat.format(date).toString() + ".png");
		if(!saveFile.exists()){
			try {
